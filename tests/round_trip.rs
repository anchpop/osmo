//! Live round-trip test for osmo against a real S3-compatible
...
//! export OSMO_TEST_BUCKET=osmo-cache-test
...
use osmo::{ensure_pulled, flush, Bucket};
...
fn unique_prefix() -> String {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    format!("osmo-test/{}-{}", std::process::id(), nanos)
}
...
/// `.osmo.json` is unioned (not clobbered) across machines, and the settings file
...
async fn r2_json_merge_unions_entries() {
    ...
    let a = tempdir();
    write_file(a.path(), ".osmo.json", settings).await;
...
fn tempdir() -> TempDir {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("osmo-cache-{}-{}", std::process::id(), nanos));
